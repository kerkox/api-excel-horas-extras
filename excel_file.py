...
def readAllSheets(filename):
    ...
    try:
        ...
        print(f"Ocurrio un error al leer el archivo, archivo no valido \"{filename}\"")
        print(e)
        return None
...
def create_dictionary_extra_hours(file_name: str):
    ...
    result["list_extra_hours"] = list_extra_hours
    return result


def extraer_data_excel(list_files):
    result = {"file_errors": [], "horas_extras": []}

    horas_extras = []
    for file_excel in list_files:
        result_dictionary = create_dictionary_extra_hours(file_excel)
        if result_dictionary["errors"] == "":
            horas_extras += result_dictionary["list_extra_hours"]
        else:
            result["file_errors"].append(result_dictionary["errors"])
            print("result_dictionary: ", result_dictionary)
            print("result: ", result)
    result["horas_extras"] = horas_extras
    return result


def generar_listas_por_tipo_hora_extra_con_cedula(list_files):
    result_data_excel = extraer_data_excel(list_files)
    data = result_data_excel["horas_extras"]
    info_horas_extras = {}
    ...
    return (info_horas_extras, result_data_excel["file_errors"])
...
def generate_txt_file_data_extra_hours(list_files):
    (result, file_errors) = generar_listas_por_tipo_hora_extra_con_cedula(list_files)
    for info in result:
        write_txt_file_by_type_extra_hour(result[info], info)
    code = zip_files_extra_hours_txt()
    return (code, file_errors)
...
# files_name = ['./uploads-excel/fail_1.xls']
# generate_txt_file_data_extra_hours(files_name)
